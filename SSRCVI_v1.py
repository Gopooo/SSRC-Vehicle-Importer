...
def validate_target_layout(buf: bytes, model):
    """Validate all vehicle descriptors.

    Multi-descriptor LODs are supported, and each descriptor may independently
    use the static16 or skin12 layout.
    """
    lod_kinds = []
    for li, L in enumerate(model["lods"]):
        ...
        kinds = []
        for di, d in enumerate(L["descs"]):
            lay = sk_geo.solve_layout(buf, L["geo"], d)
            slots = sk_geo.slot_layout(d["fmt"], lay)
            if (lay["split"] and d["stride"] == 16 and lay["clroff"] == 0
                    and lay["uvoff"] == 4 and lay["skinoff"] == 8
                    and slots == ["pos", "nrm", "clr", "uv0"]):
                kinds.append("static16")
            elif (lay["split"] and d["stride"] == 12 and lay["clroff"] is None
                    and lay["uvoff"] == 0 and lay["skinoff"] == 4
                    and slots == ["pos", "nrm", "uv0"]):
                kinds.append("skin12")
            else:
                raise ValueError(f"Unsupported target layout LOD {li} descriptor {di}:\n  descriptor={d}\n  layout={lay}\n  slots={slots}")
        lod_kinds.append(kinds)
    return lod_kinds
...
def import_static(
    ...
    layout_kinds = validate_target_layout(original, model)
    ...
    print("Layouts      : " + " | ".join("+".join(x) for x in layout_kinds))
    ...
    # ---------------- per-LOD 1388 / 1389 + descriptors
    # True multi-descriptor rebuild. EarthTruck requires every descriptor to
    # remain populated. 0x1771 positions/normals are shared by the LOD, while
    # each descriptor owns a local 0x1388 record slice (voff) and a local DL
    # slice (dloff). UV/color indices are local to that descriptor's 1388 slice.
    for li, L in enumerate(model["lods"]):
        geo = L["geo"]
        block = descriptor_blocks[li]
        kinds = layout_kinds[li]

        # Keep the full custom position/normal pool shared by all descriptors.
        byoff[geo[0x1771][0] - 16].data = bytes(p1771)

        # Split triangles between descriptors. Descriptor 0 in EarthTruck is a
        # small auxiliary section, so keep roughly the original record ratio;
        # descriptor 1 receives the rest. For generic N-descriptor targets the
        # same proportional strategy is used.
        tris_all = np.asarray(I, dtype=np.int64).reshape(-1, 3)
        weights = np.array([max(1, int(d.get("nv", 1))) for d in L["descs"]], dtype=float)
        weights /= weights.sum()
        cuts = np.rint(np.cumsum(weights) * len(tris_all)).astype(int)
        cuts[-1] = len(tris_all)
        tri_groups = []
        a0 = 0
        for cut in cuts:
            cut = max(a0, min(len(tris_all), int(cut)))
            tri_groups.append(tris_all[a0:cut])
            a0 = cut
        # Loader dislikes empty descriptors: steal one triangle from the largest
        # group if rounding ever produced an empty slice.
        for di in range(len(tri_groups)):
            if len(tri_groups[di]) == 0 and len(tris_all):
                donor = max(range(len(tri_groups)), key=lambda k: len(tri_groups[k]))
                if len(tri_groups[donor]) > 1:
                    tri_groups[di] = tri_groups[donor][-1:].copy()
                    tri_groups[donor] = tri_groups[donor][:-1]

        arena1388 = bytearray()
        dl = bytearray()
        desc_meta = []

        for di, (d, kind, tris) in enumerate(zip(L["descs"], kinds, tri_groups)):
            voff = len(arena1388)
            dloff = len(dl)
            djoint = int(d["pal"][0]) if d.get("pal") else int(joint)

            # Local 1388 records only for vertices referenced by this descriptor.
            # Display-list pos/nrm indices remain global 1771 indices; clr/uv are
            # local indices into the descriptor record slice.
            used = []
            remap = {}
            for tri in tris:
                for vi0 in map(int, tri):
                    if vi0 not in remap:
                        remap[vi0] = len(used)
                        used.append(vi0)
            ...
            for vi0 in used:
                uv = UV[vi0]
                quv = np.clip(np.rint(uv * 1024.0), -32768, 32767).astype(int)
                if kind == "static16":
                    arena1388 += bytes((255, 255, 255, 0x19))
                    arena1388 += struct.pack(">2h", int(quv[0]), int(quv[1]))
                else:
                    arena1388 += struct.pack(">2h", int(quv[0]), int(quv[1]))
                arena1388 += bytes((djoint & 255, 0, 0, 0, 255, 0, 0, 0))
            ...
            for tri in tris:
                ...
                for vi0 in map(int, tri):
                    ri = remap[vi0]
                    if kind == "static16":
                        dl += struct.pack(">4H", vi0, vi0, ri, ri)
                    else:
                        dl += struct.pack(">3H", vi0, vi0, ri)

            dllen = len(dl) - dloff
            desc_meta.append((voff, len(used), dloff, dllen, djoint))

        align32(dl)
        p1388_lod = struct.pack(">I", len(arena1388)) + bytes(arena1388)
        p1389_lod = struct.pack(">I", len(dl)) + bytes(dl)
        byoff[geo[0x1388][0] - 16].data = bytes(p1388_lod)
        byoff[geo[0x1389][0] - 16].data = bytes(p1389_lod)

        # Inline skin records are authoritative for these split layouts. Keep a
        # valid fallback 1770 table for loaders that still inspect it.
        if 0x1770 in geo:
            byoff[geo[0x1770][0] - 16].data = bytes(p1770)

        for di, (c, meta) in enumerate(zip(block, desc_meta)):
            voff, nv, dloff, dllen, djoint = meta
            ...
            c.data = bytes(dat)
            print(f"  LOD {li} desc {di}: {kinds[di]} records={nv} tris={len(tri_groups[di])} voff={voff} dloff={dloff} joint={djoint}")
